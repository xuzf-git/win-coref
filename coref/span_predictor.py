...
class SpanPredictor(torch.nn.Module):
    ...
    def __init__(self, input_size: int, distance_emb_size: int):
        ...
        self.conv = torch.nn.Sequential(torch.nn.Conv1d(64, 4, 3, 1, 1), torch.nn.Conv1d(4, 2, 3, 1, 1))
    ...
    def get_training_data(
            self, doc: Doc,
            words: torch.Tensor) -> Tuple[Optional[torch.Tensor], Optional[Tuple[torch.Tensor, torch.Tensor]]]:
    ...
    def predict(self, doc: Doc, words: torch.Tensor, clusters: List[List[int]]) -> List[List[Span]]:
        """
        ...
        heads_ids = torch.tensor(sorted(i for cluster in clusters for i in cluster), device=self.device)

        scores = self(doc, words, heads_ids)
        starts = scores[:, :, 0].argmax(dim=1).tolist()
        ends = (scores[:, :, 1].argmax(dim=1) + 1).tolist()

        head2span = {head: (start, end) for head, start, end in zip(heads_ids.tolist(), starts, ends)}
