...
class CorefModel:  # pylint: disable=too-many-instance-attributes
    ...
    def evaluate(self,
                 data_split: str = "dev",
                 word_level_conll: bool = False) -> Tuple[float, Tuple[float, float, float]]:
        ...
        with conll.open_(self.config, self.epochs_trained, data_split) \
            ...
            for doc in pbar:
                ...
                if word_level_conll:
                    conll.write_conll(doc, [[(i, i + 1) for i in cluster] for cluster in doc["word_clusters"]], gold_f)
                    conll.write_conll(doc, [[(i, i + 1) for i in cluster] for cluster in res.word_clusters], pred_f)
                else:
                ...
                pbar.set_description(f"{data_split}:"
                                     f" | WL: "
    ...
    def run(
        ...
        for i in range(0, len(words), batch_size):
            ...
            a_scores_batch = self.a_scorer(all_mentions=words,
    ...
    def save_weights(self):
        ...
        path = os.path.join(self.config.data_dir, f"{self.config.section}"
    ...
    def _bertify(self, doc: Doc) -> torch.Tensor:
        ...
        subwords_batches_tensor = torch.tensor(subwords_batches, device=self.config.device, dtype=torch.long)
        ...
        out = self.bert(subwords_batches_tensor,
                        attention_mask=torch.tensor(attention_mask, device=self.config.device)).last_hidden_state
    ...
    def _build_optimizers(self):
        ...
        if self.config.bert_finetune:
            self.optimizers["bert_optimizer"] = torch.optim.Adam(self.bert.parameters(),
    ...
    def _clusterize(self, doc: Doc, scores: torch.Tensor, top_indices: torch.Tensor):
        antecedents = scores.argmax(dim=1) - 1  # [n_words] idx of top_indices with the max score for a word
        not_dummy = antecedents >= 0  # [n_words] bool be True which word link to a not_dummy cluster
        coref_span_heads = torch.arange(0, len(scores))[not_dummy] # the word_id for the word linked to a not_dummy cluster
    ...
    def _get_docs(self, path: str) -> List[Doc]:
        if path not in self._docs:
            ...
            cache_filename = os.path.join(self.config.data_dir, cache_filename)
    ...
    @staticmethod
    def _get_ground_truth(cluster_ids: torch.Tensor, top_indices: torch.Tensor,
    ...
    def _tokenize_docs(self, path: str) -> List[Doc]:
        ...
        with jsonlines.open(path, mode="r") as data_f:
            for doc in data_f:
                doc["span_clusters"] = [[tuple(mention) for mention in cluster] for cluster in doc["span_clusters"]]
                word2subword = []
                subwords = []
                word_id = []
                for i, word in enumerate(doc["cased_words"]):
                    tokenized_word = (token_map[word] if word in token_map else self.tokenizer.tokenize(word))
