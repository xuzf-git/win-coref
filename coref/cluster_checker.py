...
class ClusterChecker:
    ...
    def add_predictions(self, gold_clusters: List[List[Hashable]], pred_clusters: List[List[Hashable]]):
        """
    ...
    @staticmethod
    def _lea(key: List[List[Hashable]], response: List[List[Hashable]]) -> Tuple[float, float]:
        """ See aclweb.org/anthology/P16-1060.pdf. """
        response_clusters = [set(cluster) for cluster in response]
        response_map = {mention: cluster for cluster in response_clusters for mention in cluster}
        importances = []
        resolutions = []
        for entity in key:
            size = len(entity)
