...
class WinDecoder(torch.nn.Module):
    ...
    def forward(
        ...
        global_struct = torch.zeros(self.topk, n_mentions + 1)
        global_scores = torch.zeros(self.topk, n_mentions + 1)
        for windows_size in range(1, n_mentions):
            # build local_structure(d + 1) from global_structure(d)
            # local_struct_origin = global_struct.repeat(2, 1)
            local_struct = self.win_builder(mentions, doc, global_struct, global_scores, windows_size)
...
class WinBuilder(torch.nn.Module):
    ...
    def forward(self, 
        ...
        for i in range(1, n_windows + 1, self.batch_size):
            global_struct_batch = global_struct[:, i:i + self.batch_size + windows_size]  # [k, batch_size+windows_size]
            global_scores_batch = global_scores[:, i:i + self.batch_size + windows_size]
            real_batch_size = global_struct_batch.size(1) - windows_size
            # To generate a 3D tensor from a 2D tensor by sampling the 2D tensor in a sliding window fashion
            ## [k, batch_size, windows_size+1]
            local_struct_batch = global_struct_batch.unfold(dimension=1, size=windows_size + 1, step=1)
            local_scores_batch = global_scores_batch[:, -real_batch_size:]
            ## new local structure: link the last mention of windows to the first mention, 0 means link to dummy
            new_link = torch.arange(i, i + real_batch_size).view(1, -1, 1).repeat(local_struct_batch.size(0), 1, 1)
            local_struct_batch_append = torch.cat((local_struct_batch[:, :, :windows_size], new_link), dim=2)
            ## score the new local structure
            new_link_pairs = (torch.arange(i, i + real_batch_size).unsqueeze(1),
                              torch.arange(i, i + real_batch_size).unsqueeze(1) + windows_size)
            ...
            # TODO: think about the local struct's data structure for global beamsearch
            local_struct_lst.append(local_struct_batch)
        local_struct = torch.cat(local_struct_lst, dim=1)  # [k, n_windows, windows_size]
        # add new local structure, which link from the last to the first mention
        local_struct = local_struct.repeat(1, 1, 2)
