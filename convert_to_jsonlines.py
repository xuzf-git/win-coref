...
def build_jsonlines(data_dir: str, out_dir: str, tmp_dir: str) -> None:
    ...
    out = {
        split_type: jsonlines.open(os.path.join(out_dir, f"english_{split_type}.jsonlines"), mode="w", compact=True)
...
def build_one_jsonline(filename: str, parsed_sents: List[str]) -> Dict[str, Union[list, str]]:
    """
...
def convert_con_to_dep(temp_dir: str, filenames: Dict[str, List[str]]) -> None:
    ...
    cmd = ("java -cp data/downloads/stanford-parser.jar"
...
def get_conll_filenames(data_dir: str, language: str) -> Dict[str, List[str]]:
    ...
    for data_split in DATA_SPLITS:
        ...
        conll_filenames[data_split] = [
            filename for filename in get_filenames(data_split_dir) if filename.endswith("gold_conll")
        ]
...
def split_jsonlines(out_dir: str, tmp_dir: str, language: str = "english") -> None:
    ...
    multiple jsonlines. """
    to_split = {
        split_type: jsonlines.open(os.path.join(tmp_dir, f"{language}_{split_type}.jsonlines"), mode="r")
        for split_type in DATA_SPLITS
    }
    out = {
        split_type: jsonlines.open(os.path.join(out_dir, f"{language}_{split_type}.jsonlines"), mode="w", compact=True)
        for split_type in DATA_SPLITS
    }
...
def split_one_jsonline(doc: dict):
    ...
    for i in range(len(parts)):
        ...
        split_doc = {
            ...
            "head": [(h - start) if h is not None else h for h in doc["head"][start:end]],
            "clusters": []
        }
...
if __name__ == "__main__":
    argparser = argparse.ArgumentParser(description="Converts conll-formatted files to json.")
    argparser.add_argument("conll_dir", help="The root directory of conll-formatted OntoNotes corpus.")
    argparser.add_argument("--out-dir", default=".", help="The directory where the output jsonlines will be written.")
    argparser.add_argument("--tmp-dir",
                           default="temp",
                           help="A directory to keep temporary files in. Defaults to 'temp'.")
    argparser.add_argument("--keep-tmp-dir",
                           action="store_true",
